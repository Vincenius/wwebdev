...
const DeleteIcon = () => (
    <svg className="ng-delete-icon" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
export default function MenuItemsControl({ menuItems, setMenuItems }: Props) {
    const updateMenu = (key: keyof MenuLinkItem, value: string | boolean, menuPart: MenuPart, index: number) => {
        const newMenuItems = menuItems
        ;(newMenuItems[menuPart][index] as any)[key] = value
    ...
    const updateLogo = (key: keyof MenuItems['logo'], value: string | boolean) => {
        const newMenuItems = menuItems
        ;(newMenuItems.logo as any)[key] = value
        setMenuItems((prevItems) => ({ ...prevItems, ...newMenuItems }))
    }
